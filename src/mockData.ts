...
export const mockGraph: SchemaGraph = {
  ...
  relations: [
    ...
    { name: 'fk_order_items_order', fromTable: 'order_items', fromColumns: ['order_id'], toTable: 'orders', toColumns: ['id'], onDelete: 'CASCADE' },
    { name: 'fk_order_items_product', fromTable: 'order_items', fromColumns: ['product_id'], toTable: 'products', toColumns: ['id'], onDelete: 'RESTRICT' },
  ],
  layout: {
    nodes: {
      users: { x: -4, y: 1, z: -2 },
